...
// Path to mock JSON data file (data.json)
...
fs.readFile(filePath, 'utf-8', (err, data) => {
  ...
  db.serialize(() => {
    ...
    jsonData.forEach(user => {
      ...
      // Insert user to db, fetch id (needed to insert skills)
      db.run('INSERT INTO users (name, company, email, phone) VALUES (?, ?, ?, ?)', [name, company, email, phone], function (userResult) {
        const userId = this.lastID;

        skills.forEach(skill => {
          // Insert each of the user's skills to db with the obtained userID
